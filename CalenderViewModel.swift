...
//title: title, startDate: startDate, endDate: endDate, eventId: eventId
...
class CalendarManager {
    ...
    func createCalendar(name: String, ownerId: String) async {
        ...
        private func createInitialEvents(eventsCollectionRef: CollectionReference) async {
            ...
            let initialEvent1 = Event(title: "Meeting", startDate: Date(), endDate: Date(), eventId: eventId)
            do {
    ...
    func getEvents(completion: @escaping ([Event]?, Error?) -> Void) async {
        let calendarId = await AuthService.shared.getGroupId()
