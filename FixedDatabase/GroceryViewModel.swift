...
class GroceryManager {
    ...
    func createGroceryList(name: String, ownerId: String) async {
        
        let groceryId = await AuthService.shared.getGroupId()
        
        let data: [String: Any] = [
            "fruits": []
        ]

        db.collection("groceryLists").document(groceryId).setData(data){ error in
                if let error = error {
                    print("Error creating grocery list: \(error.localizedDescription)")
                } else {
                    print("Grocery List successfully created with ID: \(String(describing: groceryId))")
...
//let grocery = Grocery(name: "Bread", roommate: "Mish", completed: fa/Users/user247737/Desktop/Roomease 2/Roomease/RoomeaseApp.swiftlse, category: .grains)
