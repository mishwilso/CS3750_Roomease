...
class AuthService {
    ...
    func sendMessage(group_id: String, sender: String, text: String, received: Bool, pinned: Bool) async {
        ...
        do {
            let newMessage = MessageModel(id: "\(UUID())", group_id: houseID, sender: s, text: text, timestamp: Date(), pinned: pinned)
    ...
    func getIdList(listName: String) async -> [String:String] {
        ...
        guard let user = Auth.auth().currentUser else {
            ...
            do {
                var houseId = await getGroupId()
                let userDocument = try await db.collection("Roomease").document(houseId).getDocument()
                let docData = userDocument.data()
    ...
    func updateIdList(listName: String, newList: [String:String]) async throws {
        ...
        do {
            ...
            let houseId = await getGroupId()
            print(existingList)
            try await db.collection("Roomease").document(houseId).setData([listName: existingList], merge: true)
            } catch {
                print("failed")
            }
    }
